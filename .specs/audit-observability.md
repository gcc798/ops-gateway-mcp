...
Audit 与资源描述统一写入 `AI_OPS_GATEWAY_DATA/ai-ops-gateway.db`；资源表与 Audit 表保持逻辑隔离。规划 operations、confirmations、config_changes；operations 包含 operation_id、request_id、时间、client/tool、环境、资源、action/target、risk、decision/reason、status、影响行数、耗时、statement_hash、error。SQL 原文由 `store_statement`/`redact_literals` 控制，默认脱敏。
